<header class="bg-[#F4EBD0] px-6 py-3 flex flex-wrap items-center justify-between" style="font-family: 'Inter', sans-serif;">
    ...
    {{-- Navegación - Eliminamos los márgenes mt-6 md:mt-10 --}}
    <nav class="flex items-center">
        <ul class="flex flex-wrap gap-5 text-gray-800 font-semibold items-center">
            ...
            {{-- Dropdown Database - Eliminamos pb-3 para no desalinear --}}
            <li class="relative group">
                <a href="#" class="hover:text-black transition flex items-center">
                    Database
                    <span class="ml-1 text-xs">▼</span>
                </a>
                <ul class="absolute left-0 mt-2 hidden group-hover:block bg-[#F4EBD0] shadow-lg rounded-md w-44 py-2 text-sm z-50">
    ...
    <div class="flex items-center gap-3">
        @guest
            <a href="{{ route('login') }}" class="text-[#6B8E23] border-2 border-[#6B8E23] px-4 py-1.5 rounded-md font-semibold hover:bg-[#6B8E23] hover:text-[#F4EBD0] transition">
                Login
            </a>
            <a href="{{ route('register') }}" class="bg-[#6B8E23] text-[#F4EBD0] border-2 border-[#6B8E23] px-4 py-1.5 rounded-md font-semibold hover:bg-[#C67C48] hover:border-[#C67C48] transition">
                Register
            </a>
        @endguest
        ...
        @auth
            <div class="relative group">
                {{-- Eliminamos pb-3 aquí también --}}
                <a href="#" class="font-semibold text-gray-800 hover:text-black transition flex items-center">
                    {{ Auth::user()->name }}
                    <span class="ml-1 text-xs">▼</span>
                </a>

                <ul class="absolute right-0 mt-2 hidden group-hover:block bg-[#F4EBD0] shadow-lg rounded-md w-32 py-2 text-sm z-50">
                    <li>
