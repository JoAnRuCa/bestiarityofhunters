@extends('layouts.admin') {{-- Usamos el layout de admin --}}

@section('content')
<form id="forgeForm" action="{{ route('admin.builds.store') }}" method="POST">
    @csrf
    {{-- Inputs ocultos para la lógica del build-editor.js --}}
        ...
        <div class="flex flex-col md:flex-row items-center gap-6 mb-8">
            <div class="w-full md:w-auto">
                <h1 class="text-5xl font-black tracking-tighter uppercase italic leading-none text-center md:text-left">
                    Forge <span class="text-[#6B8E23]">Official Build</span>
                </h1>
                ...
                <div class="mt-6 flex flex-col gap-6">
                    <div class="w-full sm:w-80">
                        ...
                        <input type="text" name="name" id="buildName" 
                            ...
                            placeholder="Enter build name..." required>
                ...
                <section>
                    ...
                    <div id="playstyle_container" class="bg-white border-2 border-[#6B8E23]/10 p-6 rounded-2xl shadow-sm transition-all">
                        <textarea id="playstyleField" name="playstyle" rows="6" 
                            placeholder="Describe the tactical approach for this official build..." 
...
<div id="modal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
    <div class="bg-[#F4EBD0] w-full max-w-xl rounded-[2.5rem] shadow-2xl border-4 border-[#6B8E23] flex flex-col overflow-hidden max-h-[85vh]">
        <div class="bg-[#6B8E23] p-6 text-white flex justify-between items-center">
            <h3 class="text-2xl font-black uppercase italic">Master Armory</h3>
        ...
        <div class="p-5 bg-white/30 border-b border-[#6B8E23]/10">
            <input id="searchInput" type="text" placeholder="Filter equipment..." class="w-full bg-white border-2 border-[#6B8E23]/30 rounded-2xl py-4 px-6 font-bold outline-none text-[#2F2F2F]">
        </div>
        <div id="modalList" class="overflow-y-auto p-6 space-y-3 flex-1 bg-[#F4EBD0] custom-scrollbar"></div>
    </div>
</div>

{{-- Scripts: Asegúrate de que build-editor.js sea accesible desde admin --}}
