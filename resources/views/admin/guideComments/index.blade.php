                           ...
                           <td class="px-6 py-4 text-right rounded-r-2xl border-y-2 border-r-2 border-transparent group-hover:border-[#6B8E23]/20">
                                <div class="flex justify-end gap-3 items-center">
                                    ...
                                    @if(!$isDeleted)
                                        ...
                                        {{-- Formulario Borrado Lógico (Papelera Rojo Claro) --}}
                                    ...
                                    {{-- TRUE DELETE (Relámpago Rojo Intenso) --}}
                                    <form action="{{ route('admin.guideComments.hardDelete', $comment->id) }}" method="POST" onsubmit="return confirm('CRITICAL WARNING: This will permanently delete this comment AND ALL its replies. This action cannot be undone. Proceed?')">
                                        @csrf 
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800 transition-all duration-300 hover:scale-125 active:translate-y-1" title="True Delete (Purge All Echoes)">
