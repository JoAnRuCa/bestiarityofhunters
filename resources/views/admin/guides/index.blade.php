...
<div class="min-h-screen bg-white py-12">
    <div class="w-[95%] max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <form action="{{ route('admin.guides.index') }}" method="GET" class="w-full md:w-96 flex">
                ...
                @if($search)
                    <a href="{{ route('admin.guides.index') }}" class="ml-2 px-4 py-3 bg-red-100 text-red-600 rounded-xl flex items-center hover:bg-red-200 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </a>
                @endif
            </form>

            <a href="{{ route('admin.guides.create') }}" class="flex items-center gap-2 bg-[#C67C48] text-[#F4EBD0] px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-[#2F2F2F] transition-all shadow-lg w-full md:w-auto justify-center">
        ...
        <div class="p-8 md:p-12 rounded-3xl shadow-2xl bg-[#F4EBD0] border border-[#6B8E23]/20 text-[#2F2F2F]">
            ...
            <div class="mb-10 pb-6 border-b-2 border-[#6B8E23]/10">
                <h1 class="text-3xl font-black uppercase italic tracking-tighter text-[#2F2F2F]">Hunting Archives</h1>
                <p class="text-[#6B8E23] font-bold text-xs uppercase tracking-[0.3em] mt-1">Official Bestiary & Strategy Records</p>
            </div>
                ...
                <table class="w-full text-left border-separate border-spacing-y-3">
                    <thead>
                        <tr class="text-[#6B8E23] uppercase text-xs font-black tracking-widest">
                            <th class="px-6 pb-4">Guide Title</th>
                            <th class="px-6 pb-4">Author</th>
                            <th class="px-6 pb-4">Tags</th>
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white hover:shadow-md transition-all duration-200 group">
                            ...
                            <td class="px-6 py-4 rounded-l-2xl font-bold italic text-sm border-y-2 border-l-2 border-transparent group-hover:border-[#6B8E23]/20">
                                <a href="{{ route('admin.guides.edit', $guide) }}" class="text-[#2F2F2F] hover:text-[#6B8E23] transition-all block">
                                    {{ $guide->titulo }}
                                </a>
                            ...
                            <td class="px-6 py-4 text-sm border-y-2 border-transparent group-hover:border-[#6B8E23]/20">
                                <span class="font-bold text-[#2F2F2F]">{{ $guide->user->name ?? 'Unknown Hunter' }}</span>
                            </td>

                            {{-- Etiquetas (Relación tags) --}}
                            ...
                            <td class="px-6 py-4 text-right rounded-r-2xl border-y-2 border-r-2 border-transparent group-hover:border-[#6B8E23]/20">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('admin.guides.edit', $guide) }}" class="text-[#2F2F2F] hover:text-[#6B8E23] transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                    </a>

                                    <form action="{{ route('admin.guides.destroy', $guide) }}" method="POST" onsubmit="return confirm('¿Archivar esta guía permanentemente?')">
                                        @csrf 
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-gray-400 italic">
                                No records found in the archives for your search.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
