    ...
    <div class="max-w-5xl mx-auto p-8 md:p-12 rounded-3xl shadow-2xl bg-[#F4EBD0] border border-[#6B8E23]/20 text-[#2F2F2F]">
        ...
        <form action="{{ route('admin.guides.store') }}" method="POST" class="space-y-8">
            ...
            <div class="bg-white/30 p-6 rounded-2xl border border-[#6B8E23]/5">
                ...
                <input type="text" name="titulo" value="{{ old('titulo') }}" placeholder="E.g. Rathalos Hunting Guide" 
            ...
            <div class="bg-white/30 p-6 rounded-2xl border border-[#6B8E23]/5">
                ...
                <textarea name="contenido" rows="15" placeholder="Write the weak points, elemental effectiveness, and combat tips..." 
                          class="w-full bg-white border-2 @error('contenido') border-red-500 @else border-transparent @enderror focus:border-[#6B8E23] focus:ring-0 outline-none px-6 py-5 rounded-2xl transition-all text-[#2F2F2F] leading-relaxed shadow-sm resize-none">{{ old('contenido') }}</textarea>
