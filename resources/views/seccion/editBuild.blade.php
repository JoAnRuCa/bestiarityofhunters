        ...
        <div class="flex flex-col md:flex-row items-center gap-6 mb-8">
            <div class="w-full md:w-auto">
                <h1 class="text-5xl font-black tracking-tighter uppercase italic leading-none text-center md:text-left">
                    Edit Build <span class="text-[#6B8E23]">Architect</span>
                </h1>
                ...
                <div class="mt-6 flex flex-col gap-6">
                    <div class="w-full sm:w-80">
                        ...
                        <input type="text" name="titulo" id="buildName" 
                            ...
                            placeholder="Enter build name..." value="{{ $build->titulo }}">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex flex-col flex-1 cursor-pointer group" onclick="openSelector('{{ $slot }}')">
                                            ...
                                            <span id="{{ $slot }}_name" class="font-bold text-lg leading-none transition-colors group-hover:text-[#6B8E23]">
                                                {{-- Aquí se carga el equipo existente --}}
                                                {{ $equipments->firstWhere('tipo', $slot)->real_name ?? '— Select Piece —' }}
                                            </span>
                                        </div>
                ...
                <section>
                    ...
                    <div id="playstyle_container" class="bg-white border-2 border-[#6B8E23]/10 p-6 rounded-2xl shadow-sm transition-all">
                        <textarea id="playstyleField" name="playstyle" rows="6" 
                            ...
                            class="w-full bg-transparent outline-none font-medium text-[#2F2F2F] text-lg resize-none placeholder:opacity-30">{{ $build->playstyle }}</textarea>
                    </div>
                    ...
                    <div id="skillTotals" class="space-y-6">
                        @foreach($totalSkills as $skill)
                            <div class="skill-row">
                                ...
                                <p class="text-[10px] leading-tight font-bold uppercase opacity-80">{{ $skill['desc'] }}</p>
                            </div>
                        @endforeach
                        @if($totalSkills->isEmpty())
...
<div id="modal" class="hidden fixed inset-0 bg-black/70 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
    <div class="bg-[#F4EBD0] w-full max-w-xl rounded-[2.5rem] shadow-2xl border-4 border-[#6B8E23] flex flex-col overflow-hidden max-h-[85vh]">
        <div class="bg-[#6B8E23] p-6 text-white flex justify-between items-center">
            ...
            <button type="button" onclick="closeModal()" class="text-white font-bold text-3xl hover:scale-110">×</button>
        </div>
