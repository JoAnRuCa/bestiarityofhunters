...
<div class="w-[90%] md:w-[60%] max-w-6xl mx-auto mt-12 mb-20 p-8 bg-[#F4EBD0] rounded-lg shadow-lg border border-[#6B8E23]/10">
    ...
    {{-- Buscador Estilizado --}}
    <div class="mb-10">
        <form method="GET" action="{{ route('charms.index') }}" class="flex flex-col md:flex-row gap-3">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search charm or skill..." 
    ...
    {{-- Lista de Charms en Grid --}}
    @if($paginatedCharm->count() === 0)
        <p class="text-center text-lg text-gray-600 italic py-10">No charms found in the artisan's collection.</p>
    @else
                ...
                <div class="group">
                    <a href="{{ route('charms.show', $rank['slug']) }}" 
                       class="flex items-center justify-between p-4 bg-white/50 border border-[#6B8E23]/10 rounded-lg transition-all duration-300 hover:bg-[#6B8E23] hover:text-white hover:shadow-md hover:-translate-y-1">
                        ...
                        {{-- Icono decorativo de flecha (opcional, para rellenar el espacio derecho) --}}
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
